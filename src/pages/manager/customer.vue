<template>
    <base-page>
        <template slot="header">
            <base-header title="客户列表" :show-left="true"></base-header>
        </template>
        <template slot="main">
            <div class="role-wrap">
                ...
                <div class="role-list">
                    <scroll-view class="scroll-view" :scroll-y="true" @scrolltolower="getData">
                        <ul class="list">
                            ...
                            <li @click="toOperateDetail(item)" v-for="(item,index) in list" :key="index">
...
export default {
    ...
    methods: {
        toOperateDetail(item) {
            this.$navigate({
                url: '/pages/manager/operate-detail',
                query: {
                    id: item.id
                }
            })
        ...
        getData() {
            ...
            this.$http.post(API_ADMIN_PROFIT_CUSLIST, { page: this.pageIndex, size: MAX_SIZE, key: this.searchName }).then(data => {
